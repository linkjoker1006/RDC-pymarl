...
class SoftPoliciesSelector():
    ...
    def select_action(self, agent_inputs, avail_actions, t_env, test_mode=False):
        ...
        # if test_mode and self.args.test_greedy:
        #     picked_actions = agent_inputs.max(dim=-1)[1]
        # else:
        #     picked_actions = m.sample().long()
        picked_actions = m.sample().long()
        for i in range(picked_actions.shape[0]):
            for j in range(picked_actions.shape[1]):
                while avail_actions[i][j][picked_actions[i][j]] == 0:
                    print("_________________________wrong_________________________")
                    picked_actions[i][j] = m.sample().long()[i][j]
        return picked_actions
